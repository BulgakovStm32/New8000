...
void MenuProgressBar(void){
  ...
  LcdClear();
  //Рисование рамки.
  LcdLine(  0,  0,   0, 63, PIXEL_ON);
  LcdLine(  0, 63, 127, 63, PIXEL_ON); 
  LcdLine(127, 63, 127,  0, PIXEL_ON); 
  LcdLine(127,  0,   0,  0, PIXEL_ON);
...
void MenuUpdate(MenuPageSTR_Type *menuPage, uint8_t mode, uint8_t pointStr){
  ...
  static uint8_t cursorPosition[6];// = {0, 0, 0, 0, 0};
  //--------------------
