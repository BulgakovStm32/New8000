...
void Task_RS485Protocol(void){
  ...
  PojLineFpSet    (RS485RxBuf(StatusPoj1Point));       //Сохранение состояния линий пожарных линий.
  LcFpSet         (RS485RxBuf(StatusZone1Point));      //Сохранение состояния линий Гр.
  LcDeviationSet  (RS485RxBuf(LcDeviationVolueLPoint));//Сохранение отклонения линий Гр.
  ZoneFpSet       (RS485RxBuf(StatusZone1Point));      //Сохранение состояния зон - кем были активированы зоны.
  PowerAndBatFpSet(RS485RxBuf(StatusPOWERPoint));      //Сохранение состояния питания и ЗУ.
  AmpFpSet        (RS485RxBuf(StatusPAPoint));         //Сохранение состояния УМЗЧ.
  //---------------------
  //Сохранение адреса и группы ЦП.
  MenuBackgroundPage()->PageItems.StringVar[String1] = RS485RxGetDataFromBuf(AddresPoint);
  MenuBackgroundPage()->PageItems.StringVar[String2] = RS485RxGetDataFromBuf(GroupPoint);
  //---------------------
  //Вывод на дисплей состояния питания блока.
  MenuPage(Page4)->PageItems.StringVar[String1] = (RS485RxGetDataFromBuf(StatusPOWERPoint) & PowerMask);
  MenuPage(Page4)->PageItems.StringVar[String2] =  RS485FP_GetDebugFromMB(Debug1);//Uакб
  //--------------------------------------------
  ...
  switch(Command){//Определение команды для передачи в ЦП.
      ...
      default:
        ...
        RS485FP_TxBufSet()->STR.Address         = RS485RxGetDataFromBuf(AddresPoint);
        RS485FP_TxBufSet()->STR.Group           = RS485RxGetDataFromBuf(GroupPoint);
        //RS485FP_TxBufSet()->STR.Control_Buttons = ButGetInstantState(CONTROL_BUTTONS);
        //RS485FP_TxBufSet()->STR.Preset_Buttons  = ButGetInstantState(PRESET_BUTTONS);

        MenuPage(Page1)->PageItems.StringVar[String1] = RS485RxGetDataFromBuf(AddresPoint);//адрес от ЦП
        MenuPage(Page1)->PageItems.StringVar[String2] = RS485RxGetDataFromBuf(GroupPoint); //группа от ЦП.
        TimeUTC = RS485FP_GetTimeUTCFromMB();                                              //время от ЦП.
      break;
      //-------------------- 
    }
