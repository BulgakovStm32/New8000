...
uint8_t KeyGetState(void){
  ...
  if(KeyStateReg & Key1Pin) 
		{
			CountCycle = 0;
			return UnitControlModeConst ;
		}	
  if(KeyStateReg & Key2Pin)
		{
			CountCycle = 0;
			return UnitCofigModeConst;
		}
  if(++CountCycle >= 100)
		{
			if(!(KeyStateReg & (Key1Pin | Key2Pin))) return UnitLockModeConst;
		}		
	return 0;
}
//-----------------------------------------------------------------------------
...
uint8_t MicGetState(void){
  ...
  if(++CountForMicState >= 200) //500) //100)
    {
			if(MicStateReg == (MicRxPin | MicTxPin | MicOkPin))return MicNotConnectedConst;
      if(MicStateReg == (MicRxPin | MicOkPin))           return MicFaultConst;
    }
	//--------------------
