...
//-----------------------------------------------------------------------------
#define                  SpiWaitTimeoutConst ((uint32_t)0x5000)
//static volatile uint32_t SpiWaitCount = 0;
//-----------------------------------------------------------------------------
...
void Spi2BiDirMode(uint8_t mode){
  ...
  if(mode == Spi2TxOnlyMode) SPI2->CR1 |=   SPI_CR1_BIDIOE; //Output enabled (transmit-only mode). 
...
void Spi2TxData(uint8_t data){
  ...
  {if(++SpiWaitCount > SpiWaitTimeoutConst)return;}
  SpiWaitCount = 0;
...
uint8_t Spi2RxData(void){
  ...
  //--------------------
  while(SPI2->SR & SPI_SR_BSY)
  {if(++SpiWaitCount > SpiWaitTimeoutConst)return 0;}
  SpiWaitCount = 0;
