...
static void SpeakerLine_GetSteadyStateLoop(void){
...
void SpeakerLine_MeasLoop(void){
  ...
  if(mSCount >= SP_LINE_MEAS_PERIOD)
	...
	//--------------------	
	//Получение установившегося значения после переходного процесса. 
	SpeakerLine_GetSteadyStateLoop();
	//--------------------
}
//*****************************************************************************
...
static void SpeakerRelays(uint8_t state, uint8_t relay){

	switch(relay){
		//--------------------	
		case(SP_LINE_1):
			if(state == ON) Relay_On (RELAY_SP1_ON_OFF); 
			else            Relay_Off(RELAY_SP1_ON_OFF);
		break;
		//--------------------
		case(SP_LINE_2):
			if(state == ON) Relay_On (RELAY_SP2_ON_OFF); 
			else            Relay_Off(RELAY_SP2_ON_OFF);
		break;
		//--------------------
		case(ATTEN_LINE_1):
