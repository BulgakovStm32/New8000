...
void Faults_Loop(void){
  ...
  //Линии связи с табличками ВЫХОД.
	if(SirenBoard()->Siren1.LineState != SIREN_LINE_NORM) faultReg |= Siren1FaultFlag;  
	if(SirenBoard()->Siren2.LineState != SIREN_LINE_NORM) faultReg |= Siren2FaultFlag; 
	if(SirenBoard()->Siren3.LineState != SIREN_LINE_NORM) faultReg |= Siren3FaultFlag; 
  //---------------------
