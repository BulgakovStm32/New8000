...
typedef union{
  struct{
    //--------------------
    uint16_t  NumberBadSectors; //Количество битых секторов. 
    uint16_t  NumberOfSaves;    //Количество произведенных записей
    uint16_t  CRC16;
    //--------------------
    }STR;
  uint8_t BLK[6];
}_EepHeaderUNION_TypeDef;
...
typedef union{
  struct{
    ...
    uint32_t  ErrorTimeUTC;
    uint16_t  ErrorCode;
    uint16_t  CRC16;
    //--------------------
    }STR;
  uint8_t BLK[10];
}_EepFrameUNION_TypeDef;
//-----------------------------------------------------------------------------
_EepHeaderUNION_TypeDef EepHeaderUNION;

_EepFrameUNION_TypeDef  EepErrorFrameUNION;
...
static uint8_t EepCheckWriteProgress(void){ 
  ...
  //--------------------
  if(temp & StatRegBitWIP) EepCheckWriteProgress();
...
void EepMemArrayCheck(void){
  ...
  if(temp & StatRegBitWEL)
    {
      EepHeaderUNION.STR.NumberBadSectors |= (1 << 0);
    }
  ...
  EepHeaderUNION.STR.NumberBadSectors |= (1 << 1);    
  //Пауза.
  count = 0;
  while(count < 0xFF00){count++;}     
  //--------------------
  ...
  for(count = 0; count < EepMemArrayAddrEndConst; count++)
    {
      ...
      if(temp == 0) 
        {
          EepHeaderUNION.STR.NumberBadSectors |= (1 << 2);
        }
    }
...
uint16_t EepGetNumberBadSectors(void){
  ...
  return EepHeaderUNION.STR.NumberBadSectors;
}
//-----------------------------------------------------------------------------
void EepReadSR(void){
